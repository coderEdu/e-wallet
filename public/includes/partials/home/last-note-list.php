<!-- last notes list -->
<div class="flex flex-col w-11/12 h-fit sm:w-[46.5%] md:w-[46.8%] lg:w-[47.4%] xl:w-[47.7%] 2xl:w-[47.9%] m-2 rounded-2xl mov-card-b-shadow bg-cards">
    <div class="flex flex-row w-full ml-4 justify-center items-center">
        <span class="flex "><i class="fa fa-clipboard text-xl text-green-500"></i></span>
        <h1 class="flex font-sans text-base w-full mx-auto content-start px-3 py-3 text-slate-800">Tus últimas notas</h1>
    </div>
    <div class="flex w-full h-0.5 border-b-2 border-gray-200"></div>

    <!-- first note card -->
    <a href="#" class="flex w-full h-auto justify-center items-center py-4 px-6 mb-1 card-box-shadow card-box-shadow:hover bg-white"> 
        <div class="flex w-fit h-fit mr-4">
            <i class="fa fa-clipboard flex text-5xl text-green-500"></i>
        </div>

        <div class="flex flex-col w-full divide-y-2">
            
            <div class="flex flex-col sm:flex-row space-y-1 w-full justify-between">
                <div class="flex flex-wrap w-3/4 pt-1 gap-2">
                    <span class="text-zinc-600">Tengo que activar la tarjeta BNA.</span>
                </div>
                
            </div>
            <div class="flex w-full justify-between flex-wrap">
                <h3 class="flex font-serif mb-1 text-sm text-zinc-500">30 oct 2022 17:51:34</h3>
            </div>
        </div>
    </a>

     <!-- second note card -->
     <a href="#" class="flex w-full h-auto justify-center items-center py-4 px-6 mb-1 card-box-shadow card-box-shadow:hover bg-white"> 
        <div class="flex w-fit h-fit mr-4">
            <i class="fa fa-clipboard flex text-5xl text-green-500"></i>
        </div>

        <div class="flex flex-col w-full divide-y-2">
            
            <div class="flex flex-col sm:flex-row space-y-1 w-full justify-between">
                <div class="flex flex-wrap w-3/4 pt-1 gap-2">
                    <span class="text-zinc-600">Anoche. Paddle en El Tablón.</span>
                </div>
                
            </div>
            <div class="flex w-full justify-between flex-wrap">
                <h3 class="flex font-serif mb-1 text-sm text-zinc-500">30 oct 2022 18:07:12</h3>
            </div>
        </div>
    </a>

    <!-- third note card -->
    <a href="#" class="flex w-full h-auto justify-center items-center py-4 px-6 mb-1 card-box-shadow card-box-shadow:hover bg-white"> 
        <div class="flex w-fit h-fit mr-4">
            <i class="fa fa-clipboard flex text-5xl text-green-500"></i>
        </div>

        <div class="flex flex-col w-full divide-y-2">
            
            <div class="flex flex-col sm:flex-row space-y-1 w-full justify-between">
                <div class="flex flex-wrap w-3/4 pt-1 gap-2">
                    <span class="text-zinc-600">Compré el SSD kingston 480gb.</span>
                </div>
                
            </div>
            <div class="flex w-full justify-between flex-wrap">
                <h3 class="flex font-serif mb-1 text-sm text-zinc-500">30 oct 2022 23:45:18</h3>
            </div>
        </div>
    </a>

    <div class="flex flex-row w-full justify-end items-center">
        <a href="#" class="flex items-center">
            <span class="flex"><i class="fa fa-plus text-base text-green-500"></i></span>
            <h1 class="flex w-full font-sans text-sm sm:text-md mx-auto px-3 mr-1 py-2 text-slate-800">Ver todas tus notas</h1>
        </a>
    </div>
</div>