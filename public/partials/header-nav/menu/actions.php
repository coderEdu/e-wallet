<div class="relative" x-data="{ open: false }" @mouseover="open = true" @mouseout="open = false">
    <button        
        class="flex items-center justify-center px-4 py-2 text-sm focus:outline-none hover:text-slate-800"
            ...
            <div class="relative w-full flex flex-col px-0 py-3 space-y-8">
                <ul class="space-y-0 text-sm">
                    <?php if ( $_SESSION['activate_menus'] ) { ?>
                        <li><a x-on:click="isNewDepOpen = true, seeWallets = false, open = false" href="#" class="block pl-4 hover:bg-zinc-100 py-2 text-slate-900">Depositar</a></li>
                        <li><a x-on:click="isNewExtOpen = true, seeWallets = false, open = false" href="#" class="block pl-4 hover:bg-zinc-100 py-2 text-slate-900">Extraer</a></li>
                    <?php } else { ?>
                    ...
                    <?php if ( $_SESSION['activate_menu_transfer'] ) { ?>
                        <li><a x-on:click="isNewTraOpen = true, seeWallets = false, open = false" href="#" class="block pl-4 hover:bg-zinc-100 py-2 text-slate-900">Transferir</a></li>         
                    <?php } else { ?>
